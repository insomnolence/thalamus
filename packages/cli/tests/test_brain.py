...
def test_recall_fuses_episode_with_touched_code(tmp_path: Path) -> None:
    ...
    gateway = build_two_hemisphere_gateway(repo, store=store, encoder=encoder, episodes=[episode])
    ...
    assert "module:store" in node_ids
...
def test_no_episodes_means_no_structural_section(tmp_path: Path) -> None:
    ...
    gateway = build_two_hemisphere_gateway(
        tmp_path, store=InMemoryStore(dim=32), encoder=DeterministicEncoder(dim=32), episodes=[]
    )
