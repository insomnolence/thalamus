...
def test_build_serve_gateway_scans_brain1_and_relinks(tmp_path: Path) -> None:
    ...
    gateway, returned, episodes, _, _ = build_serve_gateway(config, store=store, encoder=encoder)
...
async def test_served_mcp_path_persists_recall_and_usage_logs(tmp_path: Path) -> None:
    ...
    gateway, _, _, _, _ = build_serve_gateway(config, store=store, encoder=encoder)
