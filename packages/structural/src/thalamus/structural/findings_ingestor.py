...
class FindingsIngestor:
    ...
    def _node(
        ...
        node_id = f"finding:{self._id_prefix}{f.source_path}:{f.line}:{digest}"
        where = f"{f.source_path}:{f.line}"  # full source location — embedded + in metadata
        rule = f.rule or "finding"
        # Display label leads with rule + message (the informative bits) and a short basename:line —
        # the long source path eats the char budget and is one lookup away in metadata.
        base = f"{Path(f.source_path).name}:{f.line}"
        label = f"{rule} ({f.severity}) {base} — {f.message}".strip()[:_LABEL_CHARS]
        text = " ".join(
            part for part in (f.severity, f.tool, rule, "at", where + ":", f.message) if part
        )
